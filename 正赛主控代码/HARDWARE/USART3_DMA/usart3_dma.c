...
#define DMACLEAR                        	DMA1->HIFCR = DMA_Stream3_IT_MASK;				//TX	
...
#define USART_TX_DMA_IRQn									DMA1_Stream3_IRQn
#define USART_TX_DMA_IRQHandler						DMA1_Stream3_IRQHandler
#define USART_TX_DMA_PeripheralBaseAddr 	(uint32_t) (&(USARTx->DR))

u8 rec_data_u3[U3_DATA_LEN];      													//接收数据地址
u8 send_data_u3[U3_DATA_LEN];     													//发送数据地址
...
/****
	* @brief  串口发送
	* @param  无
	* @note   DMA_FLAG_TCIF3 没有进行宏定义 需要自行改动 数字与流的数字相同  TX
	* @retval 无
	* @bug
  * @todo	
	*/
...
{
	DMA_ClearFlag(USART_TX_DMA_Stream,DMA_FLAG_TCIF3);
...
{
    if(DMA_GetITStatus(USART_TX_DMA_Stream,DMA_IT_TCIF3) != RESET)
    {        
      DMA_ClearFlag(USART_TX_DMA_Stream,DMA_IT_TCIF3);												// 清除标志位       
...
{ 
	DMA_ClearFlag(USART_RX_DMA_Stream,DMA_IT_TCIF1|DMA_FLAG_HTIF1|DMA_FLAG_TEIF1|DMA_FLAG_DMEIF1|DMA_FLAG_FEIF1);  
	DMA_Cmd(USART_RX_DMA_Stream,DISABLE); 																			//关闭USART2 TX DMA1 所指示的通道
	USART_RX_DMA_Stream->NDTR = U3_DATA_LEN;            
...
void USART3_Upper_f_Computer(int16_t data1,int16_t data2,int16_t data3,int16_t data4,int16_t data5,int16_t data6,int16_t data7,int16_t data8,int16_t data9,int16_t data10)
{
	u8 _cnt = 0;	
	...
	USART3_DMATransfer((u32*)Send_DATA, _cnt);
}
