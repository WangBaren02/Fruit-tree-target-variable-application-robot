...
/****
  ...
  * @todo	
	*/
void UART4_DMATransfer(uint8_t BufferSRC, uint32_t BufferSize)							//UASRT DMA发送设置
{
	DMA_ClearFlag(USART_TX_DMA_Stream,DMA_FLAG_TCIF4);
	USART_TX_DMA_Stream->NDTR = BufferSize;    																	//配置BUFFER大小
	USART_TX_DMA_Stream->M0AR = (uint8_t)BufferSRC;   													//配置地址
...
void UART4_Upper_f_Computer(int16_t data1,int16_t data2,int16_t data3,int16_t data4,int16_t data5,int16_t data6,int16_t data7,int16_t data8,int16_t data9,int16_t data10)
{
	u8 _cnt = 0;	
...
//	UART4_DMATransfer((u32*)Send_DATA, _cnt);
}
